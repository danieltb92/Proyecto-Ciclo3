...
import com.usa.misiontic.proyectociclo3.repository.ReservasRepository;
...
public class ReservasService {
    ...
    public Reservation save(Reservation p){
        if(p.getIdReserva()==null){
            ...
            Optional<Reservation> e =reservasRepository.getReservas(p.getIdReserva());
            if(e.isPresent()){
                return p;
            }else{
                return reservasRepository.save(p);
    ...
    public Reservation update(Reservation p){
        if(p.getIdReserva()!=null){
            Optional<Reservation> q =reservasRepository.getReservas(p.getIdReserva());
            if(q.isPresent()){
                if(p.getIdReserva()!=null){
